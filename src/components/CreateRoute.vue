        ...
        <table class="table">
          ...
          <tbody id="tableValues">
            <tr v-for = "user in this.$store.state.users" :key="user.id">
              ...
              <td>
                <button type="button" class="btn btn-primary btn-lg" @click="changeUsertype(user.id)">
...
export default {
  ...
  methods: {
    ...
    changeUsertype(userId){
      if(confirm("Tem a certeza que quer promover/rebaixar utilizador?")){
  ...
  geocodeAddress(geocoder, resultsMap) {
    ...
    geocoder.geocode(
      ...
      this.$store.commit("ADD_ROUTE", newRoute)
    );
  },
